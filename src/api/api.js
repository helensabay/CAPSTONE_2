...
export const registerUser = async (data) => {
  try {
    ...
    return { success: false, message: error.response?.data?.message || "Registration failed or network error" };
  }
};
...
export const confirmPayment = async (orderId, method) => {
  try {
    const response = await api.post("/api/payments/confirm", { orderId, method });
